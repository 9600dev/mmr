...
9. [Layer 6: live testing under LLM monitoring](#9-layer-6-live-testing-under-llm-monitoring)
10. [Running everything](#10-running-everything)
11. [When *not* to reach for these](#11-when-not-to-reach-for-these)
...
answers the question "I want to spend $1,000 and the price is $100, so how many
whole shares?" A short [finance primer](#a-five-minute-finance-primer) below
defines terms like this.)

It passes. It will keep passing. It also tells you almost nothing, because it
checks one point in a two-dimensional input space, and you chose that point
while you already believed the code was correct.

The stakes are high here. MMR converts a dollar amount into a share count on
every automated trade, and a wrong answer spends real money. The bug that
motivated `order_math.py` had exactly this shape: an ad-hoc
`round(amount / price)` plus a "bump to at least 1 share" turned a ~$340 sized
order on a >$510 stock into a full share. That is an overspend, produced by
...
and never enter the trading container: `ty`, Hypothesis, CrossHair and mutmut
are tools you run against the code.

**`deal` is the exception, by design. It is a RUNTIME dependency.** Contracts
are not a test harness. They execute on every real call in production, so a
violated postcondition raises in the live system instead of passing a bad value
downstream. For the same reason, adding a contracted module needs a container
image rebuild, not just a code sync.

Of the dev-only four, only `ty` runs on every commit, because only `ty` is fast
...
- **Position**: how much of an instrument you currently hold, as a *signed*
  ...
  75 shares (you are **short**: you sold shares you did not own, and must buy
...
- **Notional**: the cash value of an order, `quantity × price`. A "notional of
  ...
  broker works in *shares*. Something has to convert, and the conversion loses
  money if it rounds the wrong way.
...
Catch the boring mistakes, such as a `str` where an `int` belongs. Useful,
unremarkable.

### The interesting use: making a *rule* into a *type*

This is the least obvious idea in the tutorial, so it is built up in steps.

**The rule we want to enforce:** *every order that reaches the broker must have
passed the risk gate.*

#### Step 1: the normal way, and why it leaks
...
This compiles, runs, and trades. The rule lives in the reviewer's head and in
a docstring. It is a convention, and conventions erode when code volume goes up
and review depth goes down.
...
The rule *"you must pass the gate"* has become *"you must be holding this
value"*. "Which values may go here" is precisely the question a type checker
answers. The trick is complete: a process requirement has been converted into a
data requirement.
...
That is the enforcement. Be precise about what changed: skipping the gate is
no longer a thing a reviewer might notice. It is a **type error**, produced
before the code runs, by a tool that runs on every commit. A test in the suite
runs the real type checker on that probe and asserts that it fails, so the
guarantee itself is regression-tested.

#### Step 5: Python has no private constructors

There is a hole, and it is the reason `_MINT_KEY` exists. A type checker only
checks types. Anyone can build the value directly:

```python
tok = ApprovedOrder(contract=c, order=o)      # forged, and ty accepts it!
await ex.subscribe_place_order_direct(tok)    # ...straight to the broker
```

A forged `ApprovedOrder` **is** an `ApprovedOrder`. Types give you "you
cannot pass the wrong thing". They do not give you "you cannot manufacture the
right thing". Python has no `private` keyword to lean on.

The fix is a **sentinel**: a secret value that the constructor demands, and
that exists only inside the defining module.
...
- **`is`, not `==`**: identity comparison, so a look-alike cannot satisfy it.
  An object with a permissive `__eq__` could defeat `==`.
- **`/`**: makes `_key` positional-only, so `ApprovedOrder(_key=...)` is not
  even expressible in the syntax.
...
The type system and the sentinel are complementary, and neither is enough
alone. Types stop you from passing the wrong value. The sentinel stops you from
creating the right one illegitimately. The frozen/unpicklable pair closes the
remaining routes: mutation of a token after approval, and reconstruction of a
token from a message off the wire.

> **Why not a Pydantic model?** An earlier version used one. Pydantic's
> public constructors `model_construct()` and `model_copy()` bypass `__init__`
> entirely, so a forged, sentinel-free, type-clean token was one documented API
> call away. A plain frozen `__slots__` class has no such bypass.

> **The honest threat model.** This stops accidents and refactors, such as an
> agent or a colleague wiring up a new placement path without the gate. It does
> not defend against hostile code in the same process: anything that can
> `import` the module can reach `_MINT_KEY` by reflection. True unforgeability
> needs process isolation, which is a different and much more expensive
> project. Know which threat you have actually addressed.
...
State it plainly: `mint_approved_order()` performs no verification. It is an
unconditional constructor. The type proves "someone called mint", not "the gate
ran". A new code path could mint without gating, and `ty` would accept it.
...
An exposure-increasing order must carry the gate's tri-state record,
non-empty, with nothing in the `fail` state. The check sits at the consumption
point rather than in the constructor, so it covers every future mint site
automatically, including the ones nobody remembers to audit.
...
The kernel (order construction, the risk gate, and the code that places live
orders) is held at zero diagnostics. Advisory scopes record their existing
diagnostics. A new diagnostic fails the gate, so the count can only fall.

> **In the LLM loop.** Ask a model to "add a way to place a market order from
> the dashboard" and you get a plausible, working function that calls the
> broker API directly. That is the shortest path to the stated goal, and the
> gate is somewhere else in a 2,000-line file. A type makes the shortcut
> impossible to express: there is no way to obtain an `ApprovedOrder` except
> through the gate, so the model's shortcut does not typecheck. This is the
> cheapest layer. It converts "the reviewer must notice the missing check" into
> "the code does not typecheck", and the second one scales to diffs nobody
> reads.
...
You state something that is true of **all** inputs, instead of asserting one
input/output pair. The library then hunts for a counterexample.
...
Read that as English: *either it returns a whole-share quantity whose
notional fits inside the amount, or it refuses. It never returns 0, and it
never overspends.* Hypothesis tries hundreds of combinations. When a
combination fails, it **shrinks** the counterexample to the smallest one that
still breaks:
...
### The property that catches what examples cannot

Sometimes the right property is not about a value at all. It is about an
**invariance**.

MMR's most important boolean answers this question: *does this order reduce a
position I already hold, or take on new risk?* An order that reduces one is
called **exit-class**, and exit-class orders are exempt from every safety gate,
because refusing to let someone close a position is worse than any limit the
refusal would protect. One `True` therefore switches off the trading filter,
the leverage check, the risk gate, the approval requirement, and the approver
notional tier: five gates at once. [Section 8](#8-putting-it-together) works
through the reasons in full. For now: a wrongly-`True` answer is the worst
single outcome in the system.
...
constant function. This exact property makes the following unhideable:
...
they stood at the time. Only example-based tests covered the predicate, and
every quantity those examples asserted was 150 or less, so nothing reached the
threshold. The property above did not exist yet. Writing it turned the
backdoor from invisible into two red tests.
...
gates **fail closed** (when a required input cannot be read, refuse the trade
rather than wave it through; *fail open* is the opposite); share conversion
never exceeds the sized notional; a strategy without a passing pre-deploy check
cannot start trading.

> **In the LLM loop.** This is the direct antidote to "the model tested the
> inputs it had in mind". A model that writes example tests draws from the same
> distribution it used to write the code, so its blind spots are correlated: it
> tests `qty=100` because it was thinking about `qty=100`. The backdoor above
> survived 1,882 tests for exactly this reason; every example asserted a
> quantity of 150 or less. A property does not ask the model for inputs.
> Hypothesis generates them and shrinks any failure to the minimal case. Note
> the division of labour: you state the invariant, because intent is the part a
> human must own. The library hunts for violations.

### A caveat you must internalise

Hypothesis only explores the space your strategies describe, through the
`min_value`/`max_value` bounds you hand it. It never tries anything outside
those bounds, so a bug that lives there stays invisible no matter how many
examples run.

Here is how MMR discovered that about its own sizing spec. Jumping ahead:
[section 6](#6-layer-4-mutation-testing-mutmut) introduces **mutation
testing**, which injects small deliberate bugs and then checks whether the
test suite notices. A *mutant* is one such deliberately-broken copy of the
code, for example a `>` changed to `>=`. A mutant that nothing notices marks a
gap.
...
never disagree. Only a value between 0 and 1 can tell them apart, and the
strategy never generates one. Mutating `> 0` into `> 1` is therefore
undetectable. The property is not weak; the input space is too polite.

**The property was fine. The generator was the bug.** Hold on to that
distinction: a property test can be well-written and near-useless at the same
time, if its inputs avoid the interesting region.
...
>
> **When it runs:** ⚠️ **RUNTIME: in production, on every call.** No other tool here does. A violated contract raises inside the live trading system. That is the feature, not a side effect. See [Why this is more than a fancy assert](#why-this-is-more-than-a-fancy-assert).
...
`_` in `@deal.ensure` is a namespace that holds the arguments and `result`.
These execute at runtime. Genuine output:
...
The two are different kinds of failure. A `PreContractError` blames the
caller. A `PostContractError` blames the function.

### Why this is more than a fancy assert

A contract is three things at once:

1. **Documentation that cannot rot**, because it runs.
2. **A runtime guard** in production.
3. **A test oracle**: the thing that decides whether a given run passed or
   failed. Normally your `assert` plays that role. Here the contract plays it,
   so combined with Hypothesis you do not need to write the assertion. Feed
   the function generated inputs, and the contract does the judging. In the
   run above, `deal` raised before the test's own `assert`:
...
That `@deal.ensure` **is** the safety property: *if it returns, you get at
least one whole share, and the notional never exceeds the amount.* It
constrains only the returning case. When the function raises `ValueError`,
`ensure` does not apply, which is exactly right for "refuse rather than
overspend".
...
pure core carries the precondition (and is what CrossHair verifies), while the
public function is defensively total: it validates its own inputs and fails
...
> docstring that says "never overspend" is a suggestion to a reader.
> `@deal.ensure` is a tripwire in production. It also survives refactoring:
> when a model rewrites the body of a contracted function, the contract still
> stands there and judges the result. Of everything in this tutorial,
> contracts have the best ratio of intent captured to characters typed.

The rule of thumb: contract the pure parts. `PositionSizer.compute` reads
config and portfolio state and threads a human-readable reasoning string
through many branches, which makes it a bad contract target. So its genuinely
pure sub-steps were extracted and contracted instead:
...
Each encodes "the result stays inside its documented band", the same shape
...
### Why it is worth the extra seconds

Hypothesis draws from distributions that are, sensibly, biased toward realistic
values. Some bugs live where realistic values never go. In MMR, CrossHair
found this:
...
Both `price` and `multiplier` are finite and > 0, so the precondition holds.
Their product can still underflow to exactly 0.0. For example,
`3.0765742648370966e-154 * 3.034084836703205e-308 == 0.0`. That is an
undeclared crash on the single conversion every order path uses, and random
sampling would essentially never generate it.
...
**This is the workflow:** the tool finds a counterexample, the counterexample
becomes a named deterministic test, and then the fix lands. The test outlives
the tool run.

> **In the LLM loop.** Neither a human nor a model proposes
> `3.0765742648370966e-154` as a test input. That is not a lack of skill. Both
> reason about plausible trading inputs, and the bug lives outside
...
> already written down as a contract, and you want to know whether the
> generated implementation satisfies the contract rather than merely passing
> the examples that came with it.
...
counterexample exists". It is stronger than sampling on this code, and weaker
than verification. It also only works on small, referentially-transparent
functions, which is another reason the pure kernel was extracted.
...
The idea is blunt. Deliberately break the source: flip `>` to `>=`, change a
constant, swap `and` for `or`. Then run the test suite. If the tests still
pass, they never checked that behaviour.
...
- **`killed`**: mutants where at least one test failed. Your tests noticed
  the change. This is the good column.
- **`survived`**: mutants where the whole suite still passed. Nothing you
  wrote can tell the real code from the broken code.
- **`timeout`**: the mutant made the suite hang, usually with an infinite
  loop. MMR counts these as caught, because a hang is a detected difference.
...
`proposal_transitions.py` at 100% is a 20-line state machine where every
branch matters. `position_sizing.py` at 70.9% looks alarming until you look at
where its 162 survivors live. 85 of them sit inside a `session_summary()`
reporting method, and another 6 mutate human-readable explanation strings. A
mutant that changes a log message cannot lose a single dollar. Chasing that
module to 95% means writing dozens of tests that assert the exact wording of
status text.

The useful question is never "what is the number?". It is:

> **Of the mutants that survived, which ones could change something that
> matters?**

MMR classifies by consequence: does this mutant change the number of shares
ordered, or the approve/refuse decision? For `position_sizing.py`, 70 of the
162 could touch the sized amount. Those were examined. The other 92 were
classified as cosmetic and written down as such.

A falling score is meaningful even when the absolute value is not. It means a
mutant that used to be caught no longer is. That is why the score is recorded
...
Here is a real gap this found. Five survivors clustered in `check_leverage`,
all of them missing-key defaults or guard boundaries. Nothing tested what
happens when margin data is absent. The answer turned out to be "silently
approve":
...
They differ only when `floor(amount/denom) == 1` while `1*denom > amount`.
That state is unreachable in IEEE-754. `denom > amount` implies an exact
quotient below 1, and rounding a quotient up to exactly 1.0 requires it to
exceed `1 − 2⁻⁵⁴`. Double spacing near `amount` is `2⁻⁵²` relative, so the
...
`scripts/run_mutation.sh`. Writing it down is not bureaucracy. Without it,
every future run re-litigates the same three survivors.

> **Rule:** never change production code to raise the mutation score. Only add
> tests. A survivor tells you about your *tests*.

> **In the LLM loop, this is the load-bearing layer.** Every other layer
> checks the code. Mutation testing is the only one that answers the question
> the LLM loop actually raises: the model wrote the tests too, so do they
> check anything?
>
> A worked example from the session that produced this document. Mutation
> found five survivors in `check_leverage`, so tests were written to kill
> them. On the re-run, only one of the five died. The tests looked thorough,
> asserted sensible things, and passed. Four of them exercised paths where the
> mutant and the original produce identical output. One test used a large
> `net_liquidation`, so an injected default of `1` produced a leverage near
> zero that passed the limit anyway. Both versions approved. Nothing was
> proven. The discriminating inputs had to be derived rather than guessed:
>
> ```python
...
> ```
>
> Confident-looking tests that verify nothing: that is the characteristic
...
important one. A tool that reports success when it did not run is worse than
no tool, because it manufactures confidence, and confidence is what you are
substituting for review.
...
then nothing parses, zero diagnostics count, zero regressions register, and
the gate prints "OK" with exit 0. Demonstrated with a stub that exits 1:
...
- **Score, not survivor names.** Mutant identifiers renumber whenever a
  mutated function changes. Two documented equivalents moved from `12/14` to
  `13/15` when their function grew a few lines, with nothing actually
  changing. A name-keyed baseline would cry wolf on every edit until nobody
  read it.
- **Timeouts count as caught.** A timed-out mutant made the suite hang, which
  is a detected difference. mutmut's killed-vs-timeout split is also timing
  dependent: one module moved from `57 killed / 1 timeout` to `56/2` with no
  code change, while `killed+timeout` stayed fixed at 58. Excluding timeouts
  made the score wobble ~0.1% between identical runs. In a gate, that means
  spurious failures, and then a gate nobody trusts.

### Protect the spec from the agent

`tests/invariants/` is human-owned. If one commit could edit a property and
the code it checks, an agent (or a tired human) could weaken the property to
make the code pass, and everything would stay green. The result is
...
through. The most direct way to weaken the spec was the one the guard could
not see.

Be honest about its strength: this is review hygiene, not a security
control. Two sequential commits bypass it. `--no-verify` bypasses it. Anything
that can run `git` can set the override env var. Its value is that weakening a
property becomes a separate, visible, reviewable act instead of a line buried
in a large diff.

> **In the LLM loop: here it is working.** While writing the `ApprovedOrder`
> hardening described in section 2, the obvious implementation was to make
> `mint_approved_order()` validate its arguments. That change turned
...
>
> The tempting move is to adjust the property, and an unsupervised agent
> takes it, because it makes the suite green. The policy forbids it: a red
> invariant means the implementation is wrong, unless a human revises the
> spec. So the validation moved to where the token is spent instead, which
> turned out to be the better design anyway. It covers every future mint site
> rather than the ones someone remembered to audit.
>
> The guard did not prevent a security breach. It prevented a worse outcome:
...
Every layer above is configured by a hand-maintained list with a "keep in
sync" comment and nothing that enforces it. When such a list drifts, the
failure is silent and flattering: the tool still runs, still passes, and
...
The second test is not hypothetical. Adding a new property file left the
...
took the score to 93.5%. The file existed, passed, and measured nothing.

> **In the LLM loop.** Ask a model to "add the new test file to the
> verification setup" and it adds the file. Whether it also updates four
> hand-maintained registries (the CrossHair target list, the mutation scope,
> the mutation oracle, the type-check scope) depends on whether it happened to
> know they exist. Nobody notices the omission, because the failure mode is
> silent and flattering. These wiring tests convert "the agent must remember"
> into "the suite fails until it is wired". Only the second version survives a
> hundred sessions.
...
- You hold **+75 shares** of WDS. Selling 75 takes the position to 0. That
  is an **exit**: risk removed.
- You hold **0** shares. Selling 75 takes the position to **−75**, a short.
  You now owe 75 shares you never had. That is an **open**: risk added, and
  the loss on a short has no bound, because a price can rise forever.

Both are `SELL` orders. The action word tells you nothing. Only the action
combined with the position you already hold does.

The system must treat these completely differently. MMR refuses orders for
good reasons: the position would be too large, the daily loss limit is hit,
the instrument is denylisted, the account value cannot be read. Every one of
those reasons is an argument against taking on more risk. Applied to an exit,
they become perverse: you would refuse to let someone close a losing position
because they are losing money. A blocked exit can turn a bad day into a
...
This one boolean therefore decides whether five safety gates apply at all:
...
opening order that faces no checks at all. That is why every layer in the
toolchain points at it, and why the property in section 3 states
...
| **mutmut** | Injects bugs to check that the tests notice. It first scored 61.3%: nearly 4 in 10 injected bugs went undetected, including three that flipped a fail-safe `return False` into `return True`, which is a fail-open version of the guards. After the property above joined the mutation oracle: 93.5%. The 2 remaining survivors are proven equivalent. They differ only at `position == 0.0`, which the guard above already excludes, so no input can tell them apart. |
| **`ty`** | The module is in the kernel scope, held at zero. |
| **Wiring tests** | Fail if it ever drops out of TARGETS or the mutation scope. |

Before this treatment, the predicate lived inline in a 2,000-line module.
The spec mocked it. No contract covered it. The mutation scope excluded it.
The `qty > 1000` backdoor shown earlier survived the entire suite. Now it
turns two properties red.

---

## 9. Layer 6: live testing under LLM monitoring

Every layer so far runs against the code. This layer runs the code against
reality, with the LLM watching.

The static layers share one limit: they verify properties you stated, about
components you scoped, under a model of the environment you assumed. Three
kinds of bug pass through untouched:

1. **Unstated specs.** A rule that exists as prose (in CLAUDE.md, or a
   docstring) but never became an assertion. The toolchain cannot check a
   sentence nobody encoded.
2. **Seams.** Two components that each pass their own tests, and interact
   through shared broker state that neither test suite models.
3. **Wrong environment models.** The broker's real behaviour. No analysis of
   your code can reveal facts about theirs.

Live testing closes all three, and the LLM in the monitoring seat makes it
affordable. The procedure, as MMR runs it in paper trading:

1. **Pick a path that has never executed in production.** A protective stop
   that has been placed but never triggered. A refusal gate that has never
   refused. A resize that has never run against live protective orders.
2. **Induce it, deliberately.** Buy a small position and place a stop two
   cents under the bid. Propose an order sized to breach a limit, and expect
   the refusal. Deploy a synthetic strategy that signals every bar, to force
   a pyramiding add.
3. **The LLM watches the logs, the order book, and the event store** while
   the path executes, and compares each against the documented intent.
4. **Every discrepancy becomes a fix plus a pinned regression the same day**,
...
- **Paper trading plus broker-side stops.** The account is simulated, and
  every held position carries a GTC stop order at the broker, which executes
  even if the whole software stack dies mid-test.
- **The counterexample-first rule applies here too.** A live find is not
  fixed until the failing behaviour is reproduced as a test. The live event
  gives you the property; the toolchain keeps it.

> **In the LLM loop.** The reason this layer works at all is the same agent
> occupies every seat: it watches the pulse lines, reads the event store,
> writes the fix, writes the regression, wires the toolchain, deploys, and
> verifies the deployed behaviour. The find-fix-pin-deploy-verify cycle ran
> nine times in one market day. A human team doing the same work serially
> measures that cycle in days per bug, and the live context (which log line,
> which order id, which position) has gone cold by the time the fix lands.
...
## 11. When *not* to reach for these

Proportion matters. The whole toolchain aims at a small, pure, dangerous
core: a few hundred lines where being wrong costs money.

- **Contracts** need pure functions. Do not contract something that reads
  config, hits a database, and builds a human-readable string. Extract the
  pure part and contract that.
- **CrossHair** needs small, referentially-transparent functions. It times out
  on anything wide, and a timeout tells you nothing.
- **Mutation testing** on a UI or a reporting method mostly produces string
  mutants. In MMR's own numbers, 85 of `position_sizing`'s 161 survivors live
  in a `session_summary()` reporting method and cannot affect a single traded
...
The proportionality rule follows from the loop framing: apply this where you
would not accept "the tests pass" as an answer. In MMR that is order
construction, the risk gate, position sizing, and the exit-class decision, a
few hundred lines in total. The 10,000-line CLI next door has none of it, by
choice. If a mistake means a bad table on a terminal, review-by-skimming is a
rational trade. If it means an unbounded short position, it is not.

---

## 12. Adopting this on a real project

Everything above is what the tools do. This section is how to get there, on
a new project or on one that already exists, plus what it actually cost
here.
...
Every one of these techniques has a cost that scales with surface area, and
a benefit that scales with consequence. So the first question is not "which
tool". It is "which few hundred lines would I be unable to sleep after
shipping wrong?"

In MMR that is roughly:
...
That is a few hundred lines out of tens of thousands. If your answer is
"all of it", you have not answered yet. Applying this everywhere is how the
method dies of exhaustion in week three.

A useful heuristic: where would you refuse to accept "the tests pass" as an
answer? That is the core.

### Path A: starting a new project

The order matters, because each layer makes the next cheaper.

1. **Type checker plus a fail-closed gate, on day one.** The cheapest win.
   No test-writing required. Adopting it later means burning down a backlog
   of diagnostics instead of never creating one.
2. **Write the invariants before the implementation.** This is not TDD
   exactly. You are not writing example tests. You are writing down the two
   or three sentences that must never stop being true. *"An order that
   reduces a position is never refused."* *"A returned share count never
   costs more than the budget."* Give those to the LLM as the specification.
3. **Make the dangerous decisions pure.** The single highest-leverage
   structural habit, and the one an LLM does not do unprompted: separate
   deciding from doing. A decision that takes plain values and returns a
   plain value can be contracted, property-tested, symbolically checked, and
   mutation-tested. The same logic buried inside a method that also reads a
   database can only be tested through mocks.
4. **Contracts on those pure functions** as you write them.
5. **Mutation testing once the suite exists.** Treat the first run as a
   review of your tests, not your code.
6. **CrossHair last**, on the handful of numeric functions where a
   pathological float is plausible.

### Path B: retrofitting an existing codebase

The trap is trying to reach a clean state before getting any value. Do not.
...
1. **Baseline, don't fix.** Run the type checker, record every existing
   ...
   started at 49 diagnostics and is still not zero. Zero was never the point.
   The ratchet is the point.
2. **Find the dead code first.** Retrofitting is a good excuse to delete.
   This session removed three packages (19 files, 5,673 lines) that could not
   even be imported (`ModuleNotFoundError: No module named 'arctic'`), and
   had sat next to the live code looking plausible. Verification applied to
   dead code is pure waste. Worse, dead code is a trap for an LLM grepping
   for "the CLI".
...
> codebase. That is worth more than any tutorial for convincing colleagues,
> or for giving an LLM a pattern to imitate.

### Prompts that actually change what you get back

Models default to example-based tests, and to putting logic wherever the
data already is. These instructions reliably move them, phrased as you would
type them.
...
Hypothesis generate them. A specific number in an assertion is a
sign you are still writing examples.
```
...
That single question produced the strongest test in this repo. The
exit-class answer must not depend on order quantity, so a size-triggered
backdoor has nowhere to hide. No threshold can exist inside a function whose
answer is constant in that variable.
...
That last sentence matters. Without it, you get production code
"simplified" until the mutant no longer exists, which raises the number and
destroys the signal.
...
### What it cost, and what it caught (one session, n=1)

Be straight about the evidence: this is one weekend on one codebase, with no
...
| Wiring test | A new spec file that measured zero mutants until it joined the oracle |
...
- Retrofit of the scariest predicate (extract, contract, property, mutation,
  wire into the gates): one session. It went from 61.3% to 93.5% mutation
  coverage, with the two remaining survivors proven equivalent.

**The honest counter-evidence.** The method is not free, and this session
showed its costs too:

- I proposed a "fix" to the sizer that broke 45 tests, because I had
  misunderstood which of two callers relied on the existing behaviour. The
  tests caught it. My reasoning did not. That is the system working, and it
  is also an hour spent on a change that got reverted.
- My first attempt at killing five mutants killed one. The discriminating
  inputs had to be derived numerically rather than reasoned out.
- I misread which argument a type diagnostic referred to and wrote a wrong
  explanation into a commit message. The next commit corrected it.
...
- **Chasing the mutation score.** `position_sizing` sits at 70.9%, and that
  is correct: 85 of its survivors sit in a reporting method. A rising number
  is not the goal. A specific survivor with an explanation is.
