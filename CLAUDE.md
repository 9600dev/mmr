...
**`~/.config/mmr/strategy_runtime.yaml`**: Strategy name, Python module path, class name, bar_size, conids/universe, historical_days_prior. Optional per-strategy knobs: `pyramid_max_adds` (bounded pyramiding), `trade_amount` (fixed per-open $ notional for the auto-executor, matching the backtester's live-semantics `trade_notional`; 0/absent = auto-size via the position sizer — needed when auto-sizing lands below one share on a high-priced instrument, e.g. CAT at ~$1,250).
