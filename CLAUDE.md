...
Any change under `trader/trading/` (risk gate, order construction, sizing, the proposal state machine, the gauntlet), or to a strategy, is governed by the **`mmr-verification` skill** — load it first. The layered gates, all dev-only (run via `uv run`), fastest → slowest:
