...
- **Type-check gate — `ty` (Astral).** `uv run python scripts/ty_gate.py` gates two scopes against per-scope baselines: **kernel** (`trader/trading/`, held at **0** diagnostics) and **advisory** (`trader/data/`, `trader/simulation/`, existing diagnostics baselined but can only ratchet down). A NEW diagnostic anywhere fails the gate. `--update` re-records the baselines (a human-reviewed act). The `ApprovedOrder` capability token makes "this order went through the gate" a *type* invariant here.
