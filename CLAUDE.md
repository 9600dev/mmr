...
- **Contracts + symbolic execution — `deal` + CrossHair.** `deal` pre/postconditions live on the pure kernel functions (`order_math.whole_shares_for_notional`/`_floor_shares_for_notional`, the `position_sizing` helpers, `data/proposal_transitions.py`); `uv run python scripts/crosshair_check.py` runs CrossHair symbolically over them (strictly stronger than Hypothesis's sampling — it already found a denormal-underflow bug the sampled tests missed). Also wired as a **manual-stage** pre-commit hook (too slow for every commit): `uv run pre-commit run crosshair-check --hook-stage manual`. The contracts double as runtime checks and test oracles.
