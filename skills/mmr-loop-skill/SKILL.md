...
4. **Gates apply to OPENS only; exits are never refused**: The risk gate, trading filter, leverage check, and any approval gate run on exposure-*increasing* orders. Any order that reduces the live broker position — a close, a protective stop, a position-reducing SELL — is classified **exit-class server-side** and is never blocked. (The old client `skip_risk_gate` flag is deprecated and ignored — the exit-class classifier decides, not a flag.)
5. **Gates fail closed on opens (expect brief warm-up refusals after a restart)**: When a critical input can't be read (daily PnL, account value, a price to value a market order), opening exposure is *refused* with a reason naming the missing datum. For a few seconds after a `trader_service` restart, while IB feeds warm up, market opens can be refused with *"…could not be read — fail-closed"* — this is by design, self-clears quickly, and never affects closes. Don't treat these as a fault; skip that cycle's open and retry next cycle. The hourly rate limit now counts real `ORDER_SUBMITTED` events.
6. **Proposal state machine**: Terminal statuses (EXECUTED / REJECTED / FAILED / EXPIRED) are immutable. If `approve()` fails at the broker (margin rejection, bracket rollback), OR the sized amount doesn't cover one whole share (`approve` floors amount→shares and refuses at zero — it no longer bumps to 1 share), the proposal moves to FAILED — create a new proposal rather than re-approving the old one. On a tiny amount / very high-priced instrument, raise the amount.
7. **Scanner fails loudly**: `ideas()` with `location=` raises on "no results" rather than returning an empty list, so the loop will see a real error for misconfigured markets instead of silently making zero proposals. Log the error, skip the scan for that cycle, and continue.
8. **Signed exposure in risk reports**: `portfolio_risk()` returns `net_exposure_pct`, `long_exposure_pct`, `short_exposure_pct` alongside `gross_exposure_pct`. For long-only loops these are effectively the same; for long/short loops, read `net_exposure_pct` first — correlation-cluster warnings now fire on net, so a correlated long/short hedge pair won't false-alarm.
9. **Strategy gauntlet gate ("no hash, no live")**: If the loop ever deploys or enables a strategy, an `auto_execute` strategy is refused without a PASS record from `mmr strategies gauntlet <module> --class <Class>` for its exact source hash (no override flag). Not part of the propose/approve path, but a hard boundary if you touch the roster.
10. **Opt-in approval hardening (default OFF — not on the running stack)**: Three Phase-2 defenses exist but ship disabled (see `docs/SAFETY_ROADMAP.md`). When enabled: a server-side **notional tier** requires an out-of-band `approver_key` for an above-threshold OPEN (exits exempt); `proposal_show` splits `trusted` (server-recomputed facts) from `untrusted` (proposer text) — decide only on `trusted`; and an `MMR_ROLE=proposer` context cannot approve/buy/sell/resize. Do not assume any of these are active unless configured.
11. **Context management**: Aggressive compaction keeps context at ~12K tokens. Can run indefinitely.
